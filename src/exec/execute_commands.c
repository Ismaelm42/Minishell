...
//tener en cuenta que se puede enviar un comando por terminal que no contenga comando ejecutable:
//ls -l | >outfile     =>se puede enviar perfectamente. Por lo tanto a veces hay que hacer execve
//y otras es posible que no.
int	child_process(t_global *global, int **fd, int n)
{
	...
	/*else*/ if (dup2(fd[n][0], STDIN_FILENO) == -1)
	...
	/*else*/ if (dup2(fd[n + 1][1], STDOUT_FILENO) == -1)
		return (ft_putstr_fd("minishell: error redirecting pipe\n", 2), 1);
	close(fd[n + 1][1]);
